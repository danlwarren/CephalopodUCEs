...
for ALN in "$ALIGN_DIR"/CIAlign_uce-*.nexus; do
    ...
    TMP_RAW=$(mktemp --suffix=.fa)
    seqmagick convert "$ALN" "$TMP_RAW"         # nexus→fasta

    SUB_FASTA=$(mktemp --suffix=.fa)
    # keep only headers present in SPECIES_LIST
    awk 'NR==FNR {gsub(/[ \t\r]+/, "", $0); sp[$0]=1; next}
         /^>/     {name=substr($0,2); keep=(name in sp)}
         keep' "$SPECIES_LIST" "$TMP_RAW" > "$SUB_FASTA"

    rm -f "$TMP_RAW"                            # no longer needed
    ...
    # ------------------------------------------------------
    # 2. Alignment length (all seqs same length)
    LEN=$(seqmagick info --tsv "$SUB_FASTA" | awk 'NR==2{print $4}')

    CORE_LEN=$(( LEN < 180 ? LEN : 180 ))
    START=$(( (LEN - CORE_LEN) / 2 + 1 ))
    ...
    # ------------------------------------------------------
    # 3. IUPAC consensus of the core
    CONS_FA=$(mktemp --suffix=.fa)
    consambig -sequence "$CORE_ALN" -outseq "$CONS_FA" \
              -name tmp 2>/dev/null
    ...
    if   (( CORE_LEN < 120 )); then
        ...
        echo ">${LOCUS}_p1"   >> "$OUT_FASTA"
        echo "$CONS"          >> "$OUT_FASTA"
    elif (( CORE_LEN < 180 )); then
        echo ">${LOCUS}_p1"   >> "$OUT_FASTA"
        echo "${CONS:0:120}"  >> "$OUT_FASTA"
        echo ">${LOCUS}_p2"   >> "$OUT_FASTA"
        echo "${CONS: -120}"  >> "$OUT_FASTA"
    else
        echo ">${LOCUS}_p1"   >> "$OUT_FASTA"
        echo "${CONS:0:120}"  >> "$OUT_FASTA"
        echo ">${LOCUS}_p2"   >> "$OUT_FASTA"
        echo "${CONS:60:120}" >> "$OUT_FASTA"
    fi
done
